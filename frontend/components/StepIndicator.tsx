          ...
          <React.Fragment key={step.label}>
            <button
              ...
              className={`flex items-center gap-2.5 py-1.5 px-3 rounded-md transition-all focus:outline-none ${
                isFuture ? 'cursor-not-allowed opacity-50' : 'hover:bg-slate-100 cursor-pointer active:scale-95'
              }`}
            >
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold transition-all duration-300 ${
                isCurrent ? 'bg-slate-900 text-white shadow-md ring-2 ring-slate-900/10' : 
            ...
            {idx < steps.length - 1 && (
              <div className={`w-8 md:w-16 lg:w-20 h-[1px] transition-colors duration-500 ${

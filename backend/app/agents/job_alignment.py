...
class JobAlignmentAgent(BaseAgent):
    ...
    def _extract_resume_payload(self, input_data: AgentInput) -> dict[str, Any]:
        """Extract resume payload from input data."""
        if input_data.resume is not None:
            return input_data.resume.model_dump(exclude_none=True)
        if input_data.resume_document is not None:
            return input_data.resume_document.model_dump(exclude_none=True)
        return {}

    def _filter_alignment_locations(
        self,
        ...
        """Filter skills and experience matches by valid resume locations."""
        ...
        removed_skills = len(skills_match) - len(filtered_skills)
        removed_experience = len(experience_match) - len(filtered_experience)
        structured_result["skillsMatch"] = filtered_skills
        structured_result["experienceMatch"] = filtered_experience
        return removed_skills, removed_experience

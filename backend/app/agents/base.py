...
class BaseAgent(ABC, BaseAgentProtocol):
    ...
    def _load_mock_responses(cls) -> dict[str, Any]:
        ...
        try:
            if not cls.MOCK_RESPONSES_FILE.exists():
                logger.error(f"Mock responses file does not exist at expected path: {cls.MOCK_RESPONSES_FILE}")
                return {}
    ...
    def _process_llm_result(
        ...
        """Shared template: get LLM response, parse and validate into a report model.

        Args:
            input_text: Prompt to send to the LLM.
            context: Session context for the LLM call.
            mock_response_key: Key to look up in mock responses file.
            use_mock: Whether to use mock responses.
            report_model: Pydantic model class to validate the parsed JSON.

        Returns:
            Tuple of (validated report model instance, raw JSON string).
        """
        session_id = getattr(context, "session_id", "unknown")

        raw_result = self.get_mock_response_by_key(mock_response_key) if use_mock else None

        if use_mock and raw_result is None:
            logger.warning(
                f"Mock enabled but response key not found: {mock_response_key}",
                session_id=session_id,
            )
        ...
        validated = self.parse_and_validate(raw_content, report_model)
        return validated, raw_content
    ...
    def _filter_by_location(
        ...
        """Filter a list of items to those whose location exists in the resume.

        Args:
            items: List of dicts, each containing a location key.
            resume_payload: The resume data to validate locations against.
            location_key: The dict key holding the JSON path location.
        ...
        valid = [
            item
            for item in items
            if isinstance(item, dict) and resume_location_exists(resume_payload, item.get(location_key, ""))
        ]

...
class SharpGovernanceService:
    ...
    def contains_quantifiable_claim(self, text: str | None) -> bool:
        ...
        if not text:
            return False
        return any(re.search(pattern, text, flags=re.IGNORECASE) for pattern in self.QUANTIFIABLE_PATTERNS)
    ...
    def _validate_content_strength_agent(self, response: AgentResponse, metadata: dict[str, Any]) -> None:
        try:
            ...
            metadata["hallucination_check_passed"] = hallucination_risk < self.HALLUCINATION_RISK_THRESHOLD

            suggestions = content.get("suggestions")
            if isinstance(suggestions, list):
                unfaithful_count = sum(1 for suggestion in suggestions if suggestion.get("faithful") is False)
            ...
            if isinstance(achievements, list):
                metadata["has_quantified_achievements"] = any(
                    achievement.get("quantifiable") is True for achievement in achievements
                )

            skills = content.get("skills")
            if isinstance(skills, list):
                metadata["high_evidence_skills_count"] = sum(
                    1 for skill in skills if str(skill.get("evidenceStrength", "")).upper() == "HIGH"
                )
    ...
    def _extract_significant_words(text: str) -> set[str]:
        _SIGNIFICANT_WORD_MIN_LENGTH = 3
        tokens = re.split(r"[^a-z0-9]+", text.lower())
        return {token for token in tokens if len(token) > _SIGNIFICANT_WORD_MIN_LENGTH}
    ...
    def _contains_new_proper_nouns(original: str, generated: str) -> bool:
        ...
        original_nouns = {match.lower() for match in proper_noun_pattern.findall(original)}
